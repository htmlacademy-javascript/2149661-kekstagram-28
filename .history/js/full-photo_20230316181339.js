...
const renderComments = (commentItem) => {
  // const [data] = picturesData.filter((comment) => picture.url === element.getAttribute('src'));
  // const [comments] = picturesData.filter((cmt) => picture.url === element.getAttribute('src'));
  const commentsFrament = document.createDocumentFragment();
  const newArr = arr.from(picturesData);
  console.log(newArr);
  // const commentsFrament = document.createDocumentFragment();
  // const comment
