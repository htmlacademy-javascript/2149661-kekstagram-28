...
const renderComments = (comments) => {
  ...
  if(loadMoreComment) {
    loadMoreComment.addEventListener('click', () => {
      const nextComments = getMoreComments(comments, slicedComments, ADD_COMMENTS_COUNT);
      slicedComments = [...slicedComments, ...nextComments];
      const content = commentsCreator(nextComments);
      console.log('commentsArea ',commentsArea);
