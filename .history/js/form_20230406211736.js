...
const onClickEmptyAreaMessage = (evt) => {
  // if (!evt.target.classList.contains('.success_inner') && !evt.target.classList.contains('.error_inner'))
  if(!sucessFormTemplate.contains(evt.target)){
    console.log(sucessFormTemplate);
    closeMessage();
    document.removeEventListener('click', onClickEmptyAreaMessage);
  }
};

const modalMessageHandlers = () => {
  document.addEventListener('click', onClickEmptyAreaMessage);
  addBodyFixedClass();
};
